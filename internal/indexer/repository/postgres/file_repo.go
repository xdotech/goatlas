...
func (r *FileRepo) Upsert(ctx context.Context, f *domain.File) error {
	row := r.pool.QueryRow(ctx, `
		INSERT INTO files (path, module, hash, last_scanned)
		VALUES ($1, $2, $3, now())
		ON CONFLICT (path) DO UPDATE
			SET module = EXCLUDED.module,
			    ...
			    last_scanned = now()
		RETURNING id
	`, f.Path, f.Module, f.Hash)
	return row.Scan(&f.ID)
}

// GetByPath returns the file with the given path, or nil if not found.
func (r *FileRepo) GetByPath(ctx context.Context, path string) (*domain.File, error) {
	f := &domain.File{}
	err := r.pool.QueryRow(ctx, `
		SELECT id, path, module, hash, last_scanned FROM files WHERE path = $1
	`, path).Scan(&f.ID, &f.Path, &f.Module, &f.Hash, &f.LastScanned)
